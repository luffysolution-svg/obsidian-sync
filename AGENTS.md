...
- **Key features**: 幂等同步 + Notion 增量哈希缓存（v1.5.0，`--force` 全量）；清理/回滚同步产物（v1.5.1：Notion `--clean` 递归归档、飞书 `drive +delete`、ima 客户端手动删）；飞书/Notion → Obsidian 反向导入（v1.6.x：`sync_feishu_to_obsidian.cjs` / `sync_notion_to_obsidian.cjs`，图片自动本地化）
