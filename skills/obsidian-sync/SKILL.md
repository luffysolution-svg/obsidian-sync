...
name: obsidian-sync
version: 1.1.0
description: "把 Obsidian 笔记（Markdown + 附件）单向同步到飞书云文档/知识库 和 腾讯 ima 知识库。自动检测并安装 lark-cli 与所需技能，飞书走设备流二维码登录，ima 走 Client ID/API Key 引导配置；探测目标已有结构，询问落点，支持目录层级、本地图片与附件导入，完成后校验并回传结果。跨平台（Windows/macOS/Linux）。"
metadata:
  requires:
    bins: ["node", "lark-cli"]
...
把本地 Obsidian vault（`*.md` + 附件）**单向**发布为飞书在线文档，或导入腾讯 ima 知识库。**Obsidian 是源，飞书/ima 是输出**；不回写 Obsidian，不做双向合并。
...
- [`references/env-and-auth.md`](references/env-and-auth.md) — 环境检测、依赖安装、飞书认证、ima 凭证引导
...
- [`scripts/sync_vault_to_ima.cjs`](scripts/sync_vault_to_ima.cjs) — 一键目录同步
